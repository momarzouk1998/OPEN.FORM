...
### 35. Email Verification تظهر فقط لـ Domains معروفة
- **الملف:** `src/app/register/page.tsx`
- **الوصف:** زر "إرسال كود التحقق" يظهر فقط لو `emailProvider` مش فاضي — وده بيتحدد بـ domains محددة (gmail, outlook, icloud, yahoo). أي إيميل شركة أو domain غير معروف مش بيظهر الزر خالص، والمستخدم مش قادر يكمل التسجيل.
- **الكود:**
  ```ts
  ...
  ```
- **الحل:** إظهار الزر لأي إيميل صحيح بغض النظر عن الـ domain.
...
> آخر تحديث: تم إصلاح 16 عيب في 3 دفعات. (الدفعة 2: توحيد الـ constants في ملف مشترك. الدفعة 3: استبدال `alert()` بنظام toast.)
