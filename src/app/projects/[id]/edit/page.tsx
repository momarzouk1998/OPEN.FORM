  ...
  const fetchData = async (projId: string) => {
    try {
      ...
      const [projectResult, formsResult, curriculaResult, supervisorsResult, bansResult, profilesResult, projectUsersResult] = await Promise.all([
        ...
        supabase.from('project_supervisors').select('*').eq('project_id', projId).order('created_at', { ascending: false }),
        supabase.from('project_bans').select('*').eq('project_id', projId).order('created_at', { ascending: false }),
        supabase.from('profiles').select('*').order('name'),
        supabase.from('user_projects').select('*').eq('project_id', projId).order('created_at', { ascending: false })
      ])
