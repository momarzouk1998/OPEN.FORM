...
function CreateLessonContent() {
  ...
  useEffect(() => {
    if (curriculumId) {
      supabase.from('curricula').select('title, project_id').eq('id', curriculumId).single().then(({ data }: { data: any }) => {
        if (data) {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {type === 'text' && (
              <div className="space-y-2">
                ...
                <RichTextEditor value={content} onChange={setContent} placeholder="اكتب المحتوى النصي هنا..." />
              </div>
            )}
