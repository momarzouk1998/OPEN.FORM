...
export default function EditCurriculumPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (row.type === 'text') {
      ...
      return (
        <div className="min-w-[400px]">
          <RichTextEditor value={contentVal} onChange={setContentVal} placeholder="اكتب المحتوى النصي هنا..." />
        </div>
      )
    }
    return null
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-50">
          ...
          {addType === 'text' ? (
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <RichTextEditor
                  value={newRows[0]?.content || ''}
                  onChange={(val) => setNewRows(prev => [{ ...prev[0], content: val }])}
      ...
      {editId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-xs" onClick={cancelEdit} />
          <div className="relative bg-white rounded-2xl w-full max-w-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col max-h-[90vh]">
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-gray-150 flex items-center justify-between bg-gray-50">
              <h3 className="text-lg font-bold text-gray-900">تعديل محتوى الدرس</h3>
            ...
            {/* Modal Body */}
            <div className="px-6 py-5 overflow-y-auto space-y-4 flex-1 text-right">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm"
                  placeholder="أدخل عنوان الدرس"
                  required
                />
              </div>
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl font-medium transition-all border-2 text-sm ${
                        editType === t
                          ...
                          : 'border-gray-200 bg-gray-50 text-gray-700 hover:border-gray-300 cursor-pointer'
                      }`}
                    >
                      {t === 'video' ? '🎬 فيديو' : t === 'audio' ? '🎧 صوت' : '📝 نص'}
                    </button>
              ...
              {editType === 'video' && (
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm font-mono"
                    dir="ltr"
                    placeholder="رابط الفيديو أو المعرّف (e.g. BKcJetZvOeM)"
                  />
                </div>
              )}
              ...
              {editType === 'text' && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700">المحتوى النصي (المقال) *</label>
                  <RichTextEditor
                    value={editContent}
                    onChange={setEditContent}
                    placeholder="اكتب المقال أو المحتوى النصي هنا..."
                  />
                </div>
              )}
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-150 flex gap-3 justify-end">
              <button
                ...
                onClick={cancelEdit}
                disabled={editing}
                className="px-5 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-100 disabled:opacity-50 text-sm font-medium cursor-pointer"
              >
                إلغاء
              </button>
              <button
                ...
                onClick={handleEdit}
                disabled={editing}
                className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 disabled:opacity-50 text-sm font-medium cursor-pointer flex items-center gap-2"
              >
                {editing ? (
                  <>
