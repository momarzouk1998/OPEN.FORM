...
export default function ResultsPage() {
  ...
  async function fetchData(profile: any) {
    try {
      ...
      if (statsData && statsData.length > 0) {
        ...
        const totalScore = statsData.reduce((sum: number, r: any) => sum + (Number(r.score) || 0), 0)
        const totalMax = statsData.reduce((sum: number, r: any) => sum + (Number(r.max_score) || 0), 0)
        const avgScore = totalMax > 0 ? (totalScore / totalMax * 100) : 0

        const weekAgo = new Date()
        weekAgo.setDate(weekAgo.getDate() - 7)
        const thisWeek = statsData.filter((r: any) =>
  ...
  async function handleFormClick(form: Form) {
    ...
    try {
      ...
      // Attach profile data manually (replaces Supabase join)
      ...
      const profileMap = Object.fromEntries((responseProfiles || []).map((p: any) => [p.id, p]))
      const responsesWithProfiles = (responses || []).map((r: any) => ({
        ...r,
        profiles: profileMap[r.user_id] || null,
      }))

      let filteredResponses = responsesWithProfiles
                ...
                <div className="px-4 py-3 bg-white border-b border-gray-200 flex items-center gap-4 flex-wrap">
                  ...
                  <button
                    onClick={() => {
                      ...
                      a.href = `/api/forms/${activeForm!.id}/export`
                      a.download = ''
                      a.click()
                    }}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-green-100 text-green-700 hover:bg-green-200 transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
