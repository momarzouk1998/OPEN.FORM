...
export default function FormFiller({ form, questions, existingResponse, allUserResponses, project, userId }: FormFillerProps) {
  ...
  const validateQuestionAnswer = (q: Question, answer: any): string | null => {
    if (answer === undefined || answer === null || answer === '') {
      if (q.required) {
        return `يرجى الإجابة على السؤال: ${q.text}`
      }
      return null
    }
    ...
    if (q.type === 'text') {
      ...
      const valStr = String(answer).trim()

      if (!vt || vt === 'plain') return null

      // Email
      if (vt === 'email') {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
        if (!emailRegex.test(valStr)) {
          return 'الرجاء إدخال بريد إلكتروني صحيح (مثال: name@example.com)'
        }
      }
      ...
      if (vt === 'phone') {
        ...
        if (!phoneRegex.test(valStr)) {
          return 'الرجاء إدخال رقم هاتف صحيح (7 أرقام على الأقل)'
        }
      }

      // Name count (double, triple, quad)
      if (vt === 'name') {
        const words = valStr.split(/\s+/).filter(Boolean)
        const expectedWords = parseInt(meta.validation_value || '2', 10)
        if (words.length < expectedWords) {
          const labels: Record<number, string> = {
            2: 'ثنائي (كلمتان على الأقل)',
            3: 'ثلاثي (3 كلمات على الأقل)',
            4: 'رباعي (4 كلمات على الأقل)'
          }
          return `الرجاء إدخال الاسم ${labels[expectedWords] || `${expectedWords} كلمات`}`
        }
      }
      ...
      if (vcat === 'number') {
        ...
        const checkVal = parseFloat(meta.validation_value || '0')
        const minVal = parseFloat(meta.validation_min || '0')
        const maxVal = parseFloat(meta.validation_max || '0')

        if (vt === 'is_number') return null
        if (vt === 'whole_number') {
          if (!/^-?\d+$/.test(valStr)) {
            return 'الرجاء إدخال عدد صحيح بدون فواصل'
          }
        }
        if (vt === 'equal_to') {
          if (numValue !== checkVal) return `يجب أن تكون القيمة مساوية لـ ${checkVal}`
        }
        if (vt === 'not_equal_to') {
          if (numValue === checkVal) return `يجب ألا تكون القيمة مساوية لـ ${checkVal}`
        }
        if (vt === 'less_than') {
          if (numValue >= checkVal) return `يجب أن تكون القيمة أقل من ${checkVal}`
        }
        if (vt === 'less_than_or_equal') {
          if (numValue > checkVal) return `يجب أن تكون القيمة أقل من أو مساوية لـ ${checkVal}`
        }
        if (vt === 'greater_than') {
          if (numValue <= checkVal) return `يجب أن تكون القيمة أكبر من ${checkVal}`
        }
        if (vt === 'greater_than_or_equal') {
          if (numValue < checkVal) return `يجب أن تكون القيمة أكبر من أو مساوية لـ ${checkVal}`
        }
        if (vt === 'between') {
          if (numValue < minVal || numValue > maxVal) {
            return `يجب أن تكون القيمة بين ${minVal} و ${maxVal}`
          }
        }
        if (vt === 'not_between') {
          if (numValue >= minVal && numValue <= maxVal) {
            return `يجب أن تكون القيمة خارج النطاق من ${minVal} إلى ${maxVal}`
          }
        }
      }
      ...
      // Text checks
      if (vcat === 'text_check') {
        const matchVal = String(meta.validation_value || '')
        if (vt === 'equal_to') {
          if (valStr !== matchVal) return `يجب أن تطابق القيمة تماماً: ${matchVal}`
        }
        if (vt === 'not_equal_to') {
          if (valStr === matchVal) return `يجب ألا تطابق القيمة: ${matchVal}`
        }
        if (vt === 'contains_word') {
          if (!valStr.includes(matchVal)) return `يجب أن تحتوي الإجابة على الكلمة: "${matchVal}"`
        }
        if (vt === 'does_not_contain') {
          if (valStr.includes(matchVal)) return `يجب ألا تحتوي الإجابة على الكلمة: "${matchVal}"`
        }
      }
    }
  ...
  const setAnswers = (value: Record<string, any>) => {
    setAnswersRaw(value)
    if (Object.keys(value).length === 0) {
      setValidationErrors({})
      return
    }
    // Re-validate all changed keys
    Object.keys(value).forEach(key => {
      if (value[key] !== answers[key]) {
        const questionObj = questions.find(q => q.id === key)
        if (questionObj) {
          const err = validateQuestionAnswer(questionObj, value[key])
          setValidationErrors(errors => ({ ...errors, [key]: err || '' }))
        }
      }
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    for (const q of displayQuestions) {
      ...
      if (q.type === 'matrix') {
          ...
          for (const row of rows) {
            if (row.required) {
              ...
              if (!rowVal || (Array.isArray(rowVal) && rowVal.length === 0)) {
                newErrors[q.id] = `يرجى الإجابة على الصف "${row.text}"`
                if (!firstErrorQuestionId) firstErrorQuestionId = q.id
                break
              }
            }
          }
        ...
        if (q.required && (answer === undefined || answer === null || answer === '' || (Array.isArray(answer) && answer.length === 0))) {
          ...
          // Rule check
          const err = validateQuestionAnswer(q, answer)
          if (err) {
            newErrors[q.id] = err
            if (!firstErrorQuestionId) firstErrorQuestionId = q.id
          }
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      setError('يرجى التحقق من صحة الإجابات المدخلة.')
      setSubmitting(false)
      
      if (firstErrorQuestionId) {
        const element = document.getElementById(`q-${firstErrorQuestionId}`)
        if (element) {
          ...
          {displayQuestions.map((question, index) => (
            <div 
              ...
              className={`bg-white rounded-2xl p-6 shadow-sm border transition-all duration-200 ${
                validationErrors[question.id] ? 'border-red-300 ring-2 ring-red-500/10' : 'border-gray-100'
              }`}
            >
              ...
              {validationErrors[question.id] && (
                <div className="mt-3 flex items-center gap-1.5 text-red-600 text-sm font-medium bg-red-50 p-3 rounded-xl border border-red-100">
